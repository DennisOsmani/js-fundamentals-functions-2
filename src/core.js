...
function remainingBakeTime(time) {
  const totaltTime = bakingTime()
  return totaltTime - time
}
...
function totalTimeSpent(numberOfLayers, timeBaked) {
  const sumPrep = calculatePreparationTime(numberOfLayers)

  return sumPrep + timeBaked
}
